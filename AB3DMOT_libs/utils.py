...
def get_subfolder_seq(dataset, split):
    ...
    if dataset == 'KITTI':				# KITTI
        ...
        if split == 'val':   seq_eval = ['0000','0001']
...
def initialize(cfg, data_root, save_dir, subfolder, seq_name, cat, ID_start, hw, log_file):
    ...
    pcd_seq = os.path.join(data_root, subfolder, 'velodyne', seq_name)    
    vis_dir = os.path.join(save_dir, 'vis_debug', seq_name); #mkdir_if_missing(vis_dir)
 
    # compute the min/max frame, if no image use pcd to count
    if not is_path_exists(img_seq):
        frame_list, _ = load_list_from_folder(pcd_seq)
    else:
    ...
    # no oxts file, oxts= I4
    if not is_path_exists(oxts):
        I4 = np.eye(4)
        matrices = [I4 for _ in range(num)]
        imu_poses = np.stack(matrices, axis=0)
       
    else: imu_poses = load_oxts(oxts)                 # seq_frames x 4 x 4
